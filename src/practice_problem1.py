...
class Box(object):
    ...
    def double(self):
        ...
        #     stuff_to_add = self.contents[k]
        # temp = self.volume + len(self.contents) - len(stuff_to_add)
        # for k in range(len(stuff_to_add), temp):
        #     did_not_fit = self.contents[k]
        # return did_not_fit
    ...
    def double_then_shrink(self, new_volume):
        ...
        count = 0
        count = count + len(Box.double(self))
        count = count + len(Box.shrink(self, new_volume))
        return count
